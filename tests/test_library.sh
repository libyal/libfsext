...
LIBRARY_TESTS="bitmap block directory directory_entry error extent extents_header file_entry group_descriptor inode inode_table io_handle notify superblock";
...
run_test_with_input()
{
	...
	run_test_on_input_directory "libfsext" "${TEST_DESCRIPTION}" "default" "${OPTION_SETS}" "${TEST_EXECUTABLE}" "input" "${INPUT_GLOB}";
	local RESULT=$?;
