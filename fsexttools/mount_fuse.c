...
int mount_fuse_getxattr(
     ...
     char *value,
     size_t size,
     uint32_t position FSEXTTOOLS_ATTRIBUTE_UNUSED )
#else
int mount_fuse_getxattr(
     const char *path,
     const char *name,
...
#if defined( __APPLE__ )
	FSEXTTOOLS_UNREFERENCED_PARAMETER( position )
#endif
...
int mount_fuse_readdir(
     ...
     enum fuse_readdir_flags flags FSEXTTOOLS_ATTRIBUTE_UNUSED )
#else
int mount_fuse_readdir(
     const char *path,
     void *buffer,
...
#if defined( HAVE_LIBFUSE3 )
	FSEXTTOOLS_UNREFERENCED_PARAMETER( flags )
#endif
