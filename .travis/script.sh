...
then
	docker run -t -v "${PWD}:/libfsext" ${DOCKERHUB_REPO}:${DOCKERHUB_TAG} sh -c "cd libfsext && .travis/script_docker.sh";
...
then
	...
	then
		...
		cp AUTHORS COPYING NEWS README ${PWD}/osx-pkg/usr/share/doc/libfsext;
